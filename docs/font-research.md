...
`docs/adr/` contains a strict, numbered "one decision per file" ADR sequence (0001–0006, e.g. `0003-korean-aware-search.md`, `0005-google-for-sign-in-only.md`). This font research is broad reference material covering five separate research questions feeding into (but not itself constituting) a single atomic architecture decision, so per the task instructions it was written to `docs/font-research.md` rather than inserted into the numbered ADR sequence. If/when the actual font choice is finalized and locked in, it would be reasonable to write a short follow-up ADR (e.g. `0007-pretendard-plus-jua-fredoka-for-type-system.md`) that references this document and states the decision concisely, in keeping with the existing ADR style.
